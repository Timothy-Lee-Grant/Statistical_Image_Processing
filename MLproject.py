...
np.random.seed(25) #was 102
step_size = 0.01  #was 0.01
batch_size = 200
max_epochs = 200 #was 200

# GLOBAL PARAMETERS FOR NETWORK ARCHITECTURE
number_of_layers = 5 #was 2
width_of_layers = 16  # only matters if number of layers > 1
activation = "ReLU" if True else "Sigmoid" #old expression    activation = "ReLU" if False else "Sigmoid" 
...
def main():
  ...
  # Q7 Evaluate on Test
  ################################
  testResult = net.forward(X_test)
  outputResult = np.ones((len(testResult),1))
  for i in range(len(testResult)):
    outputResult[i] = int(testResult[i].argmax())
    

    
  # add index and header then save to file
  test_out = np.concatenate((np.expand_dims(np.array(range(len(outputResult)),dtype=int), 
axis=1), outputResult), axis=1)
  test_out = test_out.astype("int32")
  header = np.array([["id", "digit"]])
  test_out = np.concatenate((header, test_out))
  np.savetxt('testOutput.csv', test_out, fmt='%s', delimiter=',')

  myHeader = np.array(["id","digit"])
  with open("NewOutputTest.csv",'w') as file:
    #for row in outputResult:
        #row.append(row)
    writer = csv.writer(file)
    writer.writerow(myHeader)
    writer.writerow(outputResult)
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    self.grad_bias = np.ones((1,len(grad[0])))
